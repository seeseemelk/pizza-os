	  ...
	  if (checksum != xmitcsum)
	    {
	      if (remote_debug)
		{
		  kernel_log ("bad checksum.  My count = 0x%x, sent=0x%x. buf=%s\n",
...
handle_exception (int exceptionVector)
{
  ...
  if (remote_debug)
    {
      kernel_log ("vector=%d, sr=0x%x, pc=0x%x\n",
	      exceptionVector, _registers[PS], _registers[PC]);
    }
  ...
  *ptr++ = hexchars[ESP]; 
  *ptr++ = ':';
  ptr = mem2hex((char *)&_registers[ESP], ptr, 4, 0);	/* SP */
  *ptr++ = ';';

  *ptr++ = hexchars[EBP]; 
  *ptr++ = ':';
  ptr = mem2hex((char *)&_registers[EBP], ptr, 4, 0); 	/* FP */
  *ptr++ = ';';

  *ptr++ = hexchars[PC]; 
  *ptr++ = ':';
  ptr = mem2hex((char *)&_registers[PC], ptr, 4, 0); 	/* PC */
  *ptr++ = ';';
	...
	case 'P':		/* set the value of a single CPU register - return OK */
	  {
		...
		{
		  hex2mem (ptr, (char *) &_registers[regno], 4, 0);
		  strcpy (remcomOutBuffer, "OK");
		  break;
		}
